...
<body>
	...
	<h1><s:message code="label.employee.manage"></s:message></h1>
	<span class="box-ok">${msg}</span>
	<span class="box-error">${msgErr}</span>
	<form action="${rootpath}nhanvien/search" method="get">
		<div class="d-flex">
			<div class="search_container">
			<input type="text" maxlength="50" name="search"
				placeholder="<s:message code="label.employeename"></s:message>" />
			<a type="submit" style="color: white;" class="button"><i class="fa-solid fa-magnifying-glass"></i></a>
			</div>
			<div class="ml-auto">
				<a href="${rootpath}nhanvien/initInsert" class="button"><s:message code="label.addnew"></s:message></a>
			</div>
	...
	<div class="table-responsive-sm" style="overflow-x: auto;">
		...
		<c:forEach items="${lst}" var="nv">
			<tr>
				<td>${nv.id}</td>
				<td>${nv.hovaten}</td>
				<td>${(nv.gioitinh == 1)? "Male": "Female"}</td>
				<td>${nv.phanQuyen.tenquyen}</td>
				<td><fmt:formatDate value="${nv.ngaysinh}" dateStyle="MEDIUM" /></td>
				<td><fmt:formatDate value="${nv.ngayvaolam}" dateStyle="MEDIUM" /></td>
				<td><fmt:formatDate value="${nv.ngaycapnhat}"
						dateStyle="MEDIUM" /></td>
				<td>${nv.didong}</td>
				<td>${nv.dtgiadinh}</td>
				<td>${nv.email}</td>
				<td>${nv.diachi}</td>
				<td><fmt:formatNumber value="${nv.luongcoban}" type="number"
						maxFractionDigits="0" /></td>
				<td>${nv.username}</td>
				<td>${nv.password}</td>
				<td>${nv.ghichu}</td>
				<td>${(nv.trangthai == 1)? "Active" : "Non-Active"}
				<td><a href="${rootpath}nhanvien/init-update?idnv=${nv.id}"><i
						class="fa-solid fa-pen-to-square fa-lg" style="color: #2562cb;"></i></a></td>
				<td><a href="#"
					onclick="confirmDelete('${rootpath}nhanvien/delete?idnv=${nv.id}')"><i
	...
	<div class="d-flex justify-content-end fixed-bottom">
		<a href="${rootpath}/home/admin" class="button" id="trang-chu-link"><i class="fa-solid fa-house fa-xl"></i></a>
	</div>
</body>
<script>
